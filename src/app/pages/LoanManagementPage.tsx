...
export const LoanManagementPage = () => {
  ...
  const confirmReturn = () => {
    if (selectedLoan?.overdue) {
      ...
      toast.success(`Libro devuelto. Multa aplicada: $${fine.toLocaleString()} (${daysOverdue} días de retraso)`);
    } else {
          ...
          <CardContent>
            <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-sm text-blue-800">
                💡 <strong>Ayuda:</strong> Aquí puedes ver todos los préstamos activos y procesar devoluciones de libros.
              </p>
            </div>
              ...
              <TableBody>
                {loans.map((loan) => (
                  <TableRow key={loan.id} className={loan.overdue ? 'bg-red-50' : ''}>
                    ...
                    <TableCell className={loan.overdue ? 'text-red-600 font-medium' : ''}>
                      {loan.dueDate}
                    </TableCell>
                    <TableCell>
                      <Badge className={loan.overdue ? 'bg-red-500' : 'bg-green-500'}>
                        {loan.overdue ? 'Atrasado' : 'Activo'}
                      </Badge>
                    </TableCell>
                    <TableCell>
                      <Button size="sm" onClick={() => handleReturn(loan)}>
                        Procesar Devolución
                      </Button>
                    </TableCell>
                  </TableRow>
        ...
        <Dialog open={showReturnDialog} onOpenChange={setShowReturnDialog}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Confirma la devolución de "{selectedLoan?.book}"
              </DialogDescription>
            </DialogHeader>
            <div className="py-4">
              {selectedLoan?.overdue && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
                  ...
                  <p className="text-sm text-red-600 mt-1">
                    Este libro tiene 16 días de retraso. Se aplicará una multa de $16.000.
                  </p>
                </div>
              )}
            ...
            <DialogFooter>
              ...
              <Button onClick={confirmReturn} className="bg-blue-900 hover:bg-blue-800">
