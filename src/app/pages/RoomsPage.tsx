...
import { Users, DoorOpen, Clock, Calendar as CalendarIcon } from "lucide-react";
...
export const RoomsPage = () => {
  ...
  const rooms: Room[] = [
    ...
    {
      id: '2',
      name: 'Sala B - Estudio Individual',
      capacity: 4,
      equipment: ['WiFi', 'Escritorios'],
      status: 'available',
    },
    {
      id: '3',
      name: 'Sala C - Conferencias',
      capacity: 20,
      equipment: ['Proyector', 'Sistema de Audio', 'Pantalla Grande', 'WiFi'],
      status: 'available',
    },
    {
      id: '4',
      name: 'Sala D - Estudio Grupal',
      capacity: 6,
    ...
    {
      id: '5',
      name: 'Sala E - Multimedia',
      capacity: 10,
      equipment: ['Computadoras', 'Proyector', 'WiFi'],
      status: 'available',
    },
  ];

  const [existingReservations] = useState<Reservation[]>([
    {
      id: '1',
      roomId: '4',
  ...
  const handleConfirmReservation = () => {
    ...
    if (formData.startTime < getMinTime() || formData.endTime > getMaxTime()) {
      toast.error('El horario permitido es de 8:00 AM a 8:00 PM');
      return;
    }

    if (!selectedRoom || !isRoomAvailable(selectedRoom.id, formData.date, formData.startTime, formData.endTime)) {
      toast.error('La sala no está disponible en el horario seleccionado');
      return;
    }
  ...
  const filteredRooms = selectedTime
    ? rooms.filter(room => {
        ...
        endTime[0] += 1;
        const calculatedEndTime = `${endTime[0].toString().padStart(2, '0')}:${endTime[1].toString().padStart(2, '0')}`;
        return isRoomAvailable(room.id, selectedDate, selectedTime, calculatedEndTime);
      })
    : rooms;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="ml-64 pt-16 p-6">
        ...
        {filteredRooms.length === 0 && (
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <p className="text-gray-600">No hay salas disponibles para los criterios seleccionados</p>
            </CardContent>
          </Card>
        )}
        ...
        <Dialog open={showReservationDialog} onOpenChange={setShowReservationDialog}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Reservar {selectedRoom?.name}</DialogTitle>
              <DialogDescription>
            ...
            <DialogFooter>
              ...
              <Button onClick={handleConfirmReservation} className="bg-blue-900 hover:bg-blue-800">
                Confirmar Reserva
              </Button>
            </DialogFooter>
          </DialogContent>
