...
export const ReportsPage = () => {
  ...
  const filteredLoans = useMemo(() => {
    return loans.filter((loan) => {
      const user = loan.estudiante?.user;
      const matchesRole = roleFilter === "all" || users.find((item) => item.id === user?.id)?.rol === roleFilter;
      const matchesUser = userFilter === "all" || String(user?.id) === userFilter;
      return (
        withinRange(loan.fechaPrestamo) &&
        matchesRole &&
        matchesUser &&
        matchesSearch([loan.libro?.titulo, user?.nombreCompleto, user?.correoInstitucional])
      );
    });
  ...
  const filteredFines = useMemo(() => {
    return fines.filter((fine) => {
      const user = fine.prestamo?.estudiante?.user;
      const matchesRole = roleFilter === "all" || users.find((item) => item.id === user?.id)?.rol === roleFilter;
      const matchesUser = userFilter === "all" || String(user?.id) === userFilter;
      return (
        ...
        matchesRole &&
        matchesUser &&
        matchesSearch([fine.prestamo?.libro?.titulo, user?.nombreCompleto, user?.correoInstitucional, fine.estado])
      );
    });
  ...
  const filteredReservations = useMemo(() => {
    return reservations.filter((reservation) => {
      ...
      const userRole = users.find((item) => item.id === user?.id)?.rol;
      ...
      return (
        ...
        matchesRole &&
        matchesUser &&
        matchesSearch([reservation.sala?.nombre, user?.nombreCompleto, reservation.estado])
      );
    });
  ...
  const filteredGrades = useMemo(() => {
    return grades.filter((grade) => {
      const user = users.find((item) => item.id === grade.estudianteId);
      ...
      return (
        ...
        matchesPeriod &&
        matchesSubject &&
        matchesSearch([user?.nombreCompleto, user?.correoInstitucional])
      );
    });
  ...
  const triggerBackendExport = async (kind: "biblioteca" | "academico", format: "pdf" | "excel") => {
    ...
    try {
      if (kind === "biblioteca") {
        const blob = await api.downloadLoansReport({ startDate, endDate, format });
        ...
        <div className="mt-3 flex flex-col gap-3 lg:flex-row">
          <Select value={userFilter} onValueChange={setUserFilter}>
            ...
            <SelectContent className="dark:border-gray-700 dark:bg-gray-800">
              ...
              {users.map((user) => (
                <SelectItem key={user.id} value={String(user.id)}>{user.nombreCompleto}</SelectItem>
              ))}
            </SelectContent>
          </Select>
                        ...
                        <div className="space-y-3">
                          {filteredGrades.slice(0, 8).map((grade) => (
                            <div key={grade.id} className="rounded-lg bg-gray-50 p-3 dark:bg-gray-700/50">
                              <p className="font-medium text-gray-700 dark:text-white">{users.find((user) => user.id === grade.estudianteId)?.nombreCompleto || "Estudiante"}</p>
                              <p className="text-sm text-gray-500 dark:text-[#B7BDD6]">
                                {subjects.find((subject) => subject.id === grade.asignaturaId)?.nombre || "Materia"} · {grade.periodoAcademico} · Nota {Number(grade.valor).toFixed(1)}
                              </p>
                            </div>
                          ))}
                        </div>
